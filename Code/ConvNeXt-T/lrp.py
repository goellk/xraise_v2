...
from helper import ConvNextTiny_BinaryClassifier
from torchvision.models import convnext
from torchvision.models.convnext import CNBlock
from torchvision.models.convnext import LayerNorm2d
from torchvision.ops import StochasticDepth, misc
...
def apply_lrp_rules(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, ConvNextTiny_BinaryClassifier):
            module.rule = EpsilonRule(0.01)  # Use a small epsilon
            print(f"Applying EpsilonRule to custom ConvNeXtTinyMultiLabel layer: {name}")
        elif isinstance(module, convnext.ConvNeXt):
            module.rule = EpsilonRule(0.05)  # For ResNet base layers
            print(f"Applying EpsilonRule to ResNet layer: {name}")
        elif isinstance(module, LayerNorm2d):
            module.rule = EpsilonRule(0.001)
            print(f"Ignoring LayerNorm2d layer with zero epsilon: {name}")
            continue
        elif isinstance(module, torch.nn.modules.activation.Sigmoid):
            module.rule = EpsilonRule(0.001)
            ...
            module.rule = EpsilonRule(0.05)
        elif isinstance(module, StochasticDepth):
            module.rule = EpsilonRule(0.05)
        elif isinstance(module, misc.Permute):
            module.rule = EpsilonRule(0.05)
        elif isinstance(module, nn.GELU):
            module.rule = EpsilonRule(0.05)
        elif isinstance(module, CNBlock):
            module.rule = EpsilonRule(0.05)
        else:
