...
class CondAttribution:
        ...
        if target_list:
            ...
            for i, targets in enumerate(target_list):
                if isinstance(targets, list):  
                    targets = [t for t in targets if t < output_selection.shape[1]]  # Ensure valid index
                else:
                    targets = min(targets, output_selection.shape[1] - 1)  # Prevent out-of-bounds error
    ...
    #def relevance_init(self, prediction, target_list, init_rel):
    #    output_selection = prediction.clone()
    #    mask = torch.zeros_like(output_selection)
#
    #    # Ensure target_list is never None
    #    if target_list is None:
    #        target_list = [0] * prediction.shape[0]  # Set all targets to class index 0 (binary classification case)
#
    #    for i, targets in enumerate(target_list):
    #        if isinstance(targets, list):  
    #            targets = [t for t in targets if t < output_selection.shape[1]]  # Ensure valid index
